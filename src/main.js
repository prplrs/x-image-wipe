...
var sizeFactor = 12;
var width = 15 * sizeFactor;
var height = 9 * sizeFactor;
...
function hand() {
  ...
  const runDetection = () => {
    model.detect(video).then((predictions) => {
      ...
      if (length > 0) {
        if (_length === 0) window.dispatchEvent(new Event("handtrack-start"));
        _length = length;

        var innerWidth = window.innerWidth;
        window.dispatchEvent(
          new CustomEvent("handtrack", {
            detail: {
              x: hands[0].x * innerWidth,
              innerWidth: innerWidth,
            },
          })
        );
      } else {
        if (_length > 0) window.dispatchEvent(new Event("handtrack-end"));
        _length = 0;
      }
...
function init() {
  ...
  l1.load("img/prpl-bg-1.jpeg", function (img) {
    slide.setImage(img);
  });
  ...
  l2.load("img/prpl-bg-2.jpeg", function (img) {
    slide2.setImage(img);
  });
...
function createTweenScrubber(tween, seekSpeed) {
  ...
  var _hx = 0;
  window.addEventListener("handtrack-start", function (e) {
    stop();
  });
  window.addEventListener("handtrack-end", function (e) {
    resume();
  });
  window.addEventListener("handtrack", function (e) {
    var hx = e.detail.x;
    var dhx = THREE.Math.clamp(hx - _hx, seekSpeed * -1, seekSpeed);
    var realProgress = (hx + dhx) / e.detail.innerWidth;
    _hx = hx;
    var p = THREE.Math.clamp(realProgress, 0, 1);
    tween.progress(p);
  });
}
